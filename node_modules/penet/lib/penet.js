...
var Penet  = function(options){
    ...
    this.addBoot = function(url,boot){
        this.boots[url] = boot
        logger.info("Penet addboot: " + url);
    }

    this.addHandle = function(handleName, handle){
        this.handles[handleName] = handle;
        logger.info("Penet addhandle: " + handleName);
    };

    this.setEnv = function(options){
        var options = options || {};
        this.encoding = options.encoding || this.encoding;
        logger.info("Penet Encoding: " + this.encoding);
...
Penet.prototype.start =  function(port){
    ...
    var requestListener = function(incomingMessage, response){
        ...
        logger.info("Cookie: " + incomingMessage.headers["cookie"]);
        ...
        doMessage(incomingMessage,function(){
            fs.readFile(me.static_dir + url.parse(incomingMessage.url).pathname, {
                ...
                if(me.boots.hasOwnProperty(url.parse(incomingMessage.url).pathname)
                    && !(me.handles.hasOwnProperty(params.handle)
                         && me.handles[params.handle].hasOwnProperty(params.method))){
                    var boot = new Boot(me.boots[url.parse(incomingMessage.url).pathname]);

                    boot.penet              = me;
                    boot.incomingMessage    = incomingMessage;
                    boot.response           = response;
                    boot.params             = params;
                    boot.template           = template;
                    boot.static_dir         = me.static_dir;
